...
export function DateConverter() {
  const { data: date, isLoading: loadingDate } = useSAPI(
    "/date/today",
    getDate,
  );
  ...
  const showDate = (_convertedDate: ConvertedDate) => {
    ...
    setenglishDate(
      `${int_date_parts[2]}-${englishMonths.indexOf(int_date_parts[1]).toString().padStart(2, "0")}-${int_date_parts[0].toString().padStart(2, "0")}`,
    );
    setNepaliDate(
      `${nep_date_parts[2]}-${nepaliMonths.indexOf(nep_date_parts[1]).toString().padStart(2, "0")}-${nep_date_parts[0].toString().padStart(2, "0")}`,
    );
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="h-6 w-6 p-2">
          📆
        </Button>
      </PopoverTrigger>
